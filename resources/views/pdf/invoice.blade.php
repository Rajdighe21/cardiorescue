...
<body>
    ...
    <hr class='hr'>
    <hr>
    <h4 class="date"> Date: {{ \Carbon\Carbon::parse($lastRecord->regi_date)->format('d M Y') }}</h4>
    <h4 class="Info" style="margin-top: 1rem">Name : {{ $lastRecord->patient_name }}</h4>
    <h4 class="Info">Receipt No : CR0000{{ $lastRecord->id }}</h4>
    ...
    <table>
        ...
        <tbody>
             <tr>
                <td>1</td>
                <td>
                    @if (!empty($lastRecord->cost_of_ms))
                        Device Muscle Stimulator
                    @elseif(!empty($lastRecord->cost_of_us))
                        Device Ultrasound
                    @else
                        Device MS/US
                    @endif
                ...
                <td>
                    @if ($lastRecord->cost_of_ms != null)
                        {{ $lastRecord->cost_of_ms }}
                    @elseif($lastRecord->cost_of_us != null)
                        {{ $lastRecord->cost_of_us }}
                    @else
                        0
                    @endif
                </td>
            ...
            <tr>
                <td>2</td>
                <td>SMRT™ TREATMENT ({{ $lastRecord->session_numbers }} Session: {{ $lastRecord->session_numbers }}
                    Day
                    Traveling Including)</td>
                <td>{{ $lastRecord->session_numbers }}</td>
                <td>{{ $lastRecord->cost_of_session * $lastRecord->session_numbers }}</td>
            </tr>
            <tr>
                <td>3</td>
                <td>
            ...
            <tr>
                ...
                <td>Total Amount</td>
                <td></td>
                <td> {{ $lastRecord->total_amt }}</td>
            </tr>
            <tr>
                <td></td>
                <td>Discount</td>
                <td></td>
                <td> {{ $lastRecord->discount_amt }}</td>
            </tr>
